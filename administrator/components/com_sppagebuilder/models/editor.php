...
class SppagebuilderModelEditor extends AdminModel
{
	...
	public function getPages($pageData)
	{
		...
		try
		{
			...
			$query->select('p.id, p.asset_id, p.title, p.text, p.content, p.extension_view, p.catid, p.published, p.created_on, p.created_by, p.language, p.hits, p.checked_out, p.css');
